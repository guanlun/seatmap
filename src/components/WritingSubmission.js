...
export default class WritingSubmission extends React.Component {
    componentDidMount() {
        this.refs.writingContent.innerHTML = WRITINGS[Math.floor(Math.random() * WRITINGS.length)];
    }

    render() {
        return (
            <div className="writing-submission-container">
                <div className="writing-submission">
                    <div className="writing-submission-form-item form-label">Topic</div>
                    <select ref="topicSelect" className="writing-submission-form-item">
                        <option>Foodwebs</option>
                        <option>Adaptation</option>
                        <option>CO2</option>
                        <option>Inherited traits</option>
                    </select>
                    <div className="writing-submission-form-item form-label">Context</div>
                    <select ref="contextSelect" className="writing-submission-form-item">
                        <option>Home</option>
                        <option>Museum</option>
                        <option>Park</option>
                    </select>
                    <div className="writing-submission-form-item form-label">Writing submission</div>
                    <div ref="writingContent" contentEditable="true" className="writing-submission-form-item content-edit"></div>
                    <button className="writing-submission-form-item" onClick={this.handleSubmitButtonClick.bind(this)}>Submit</button>
                </div>
            </div>
        )
    }
    ...
    handleSubmitButtonClick() {
        const topic = this.refs.topicSelect.value;
        const context = this.refs.contextSelect.value;
        const writing = this.refs.writingContent.innerHTML;
